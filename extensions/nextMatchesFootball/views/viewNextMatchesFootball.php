...
<section class="nextmatchesfootball">
	<h2><?= $content['title'];?></h2>
	<div class="nextmatchesfootball-content <?php if ($slider) {?>swiper-container<?php }?>" <?php if ($slider && is_numeric($content['content']['actual_date'])){?>data-swiper="{'initialslide': '<?=$content['content']['actual_date']?>'}"<?php }?>>
		<?php if ($navigation) {?>
