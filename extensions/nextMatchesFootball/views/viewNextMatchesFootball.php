	...
	<div class="nextmatchesfootball-content <?php if ($slider) {?>swiper-container<?php }?>">
		...
		<div class="competition <?php if ($slider) {?>swiper-wrapper<?php }?>">
			...
			?>
				...
				<?php }?>
				<div class="date-matches"><?php echo $content['titles']['phases'][$key][COUNTRY_CODE];?></div>
				...
				<ul>
					...
					<li class="row match <?= $match['status']?>">
						<div class="col-12">
							<a href="/<?= strtolower($this->normalizeString($content['section'][COUNTRY_CODE]))?>/<?= strtolower($this->normalizeString($content['titles']['match'][COUNTRY_CODE]))?>/<?= strtolower($this->normalizeString($content['titles'][$content['type']][COUNTRY_CODE]))?>/<?= $content['tournament']?>/<?= $match['id']?>/<?php echo Widgets::normalizeString($match['team_local']).'-vs-'.Widgets::normalizeString($match['team_visit'])?>" name="<?php echo $match['team_local'] . ' vs ' . $match['team_visit']?>">
