	...
	<div class="newssportsgomovil-content <?php if ($slider) {?>swiper-container<?php }?>">
		<ul class="list-news <?php if ($slider) {?>swiper-wrapper<?php }?>">
			...
			<li <?php if ($slider) {?>class="swiper-slide"<?php }?>>
				<a href="/<?= strtolower($content['news'][COUNTRY_CODE]) . '/'. strtolower($content['article'][COUNTRY_CODE]) .'/'.$news['nota_id'] .'/'. Widgets::normalizeString($news['titulo']);?>" title="<?=$news['titulo']?>">
					<div class="row">
						<div class="col-12">
